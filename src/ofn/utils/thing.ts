...
import { Nullable, isDefined, isUndefNull, isNoObject } from './types';
import { UUID } from './uuid';
import { copy } from '../../components/VirtualTree/methods/copy';
...
export abstract class Thing extends EventHandler {
  ...
  buildAttrAccessors(attrnames: string[]) {
    attrnames.forEach(attr => {
      this.buildAttrAccessor(attr);
    })
  }

  buildAttrAccessor(attrname: string, defaultValue?: any) {
    let name = attrname;
    ...
    Object.defineProperty(self, name, {
      ...
      set: function (value) {
        ...
        if (value.copy && isUndefNull(self[`_${name}`])) {
          self[`_${name}`] = new value.constructor();
        }

        if (self[`_${name}`].copy)
          self[`_${name}`].copy(value);
        else
          self[`_${name}`] = value;
