...
export function extrude(options: IExtrudeOptionsEx): IGeometry {
    ...
    var up: Vec3 | Array<Vec3> = options.up as any;
    var right: Vec3 | Array<Vec3> = options.right as any;
    var newholes = [];
    ...
    for (let i = 0; i < options.path.length; i++) {
        ...
        let righti: Vec3 | any = Array.isArray(right) && right[i] || right;
        let upi: Vec3 | any = Array.isArray(up) && up[i] || up || right && v3().crossVecs(direction, righti);
        if (!right)
