...
export class Circle {
     ...
     * @param lengthAngle 
     */
    static ArcSgement(center: Vec3, radius, segment: number = 32, startAngle = 0, lengthAngle = Math.PI * 2): Vec3[] {
        const perAngle = lengthAngle / segment;
        const result = [];

        for (let i = 0; i <= segment; i++) {
            const v = v3();
            v.x = center.x + radius * Math.cos(startAngle + i * perAngle);
            v.y = center.y + radius * Math.sin(startAngle + i * perAngle);
            result.push(v);
        }
